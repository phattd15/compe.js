class SparseTable {
   ...
   */
  constructor(initValue: any, merger: any = (a: any, b: any) => a < b ? a : b) {
    let elemCount = initValue.length;
    this.logFactor = this.floorLog2(elemCount);
    this.elemCount = elemCount;
    this.merger = merger;
    this.cont = Array(elemCount).fill(0).map(() => Array(this.logFactor + 1));
    for (let i = 0; i < elemCount; i ++) {
      this.cont[i][0] = initValue[i];
    }
    for (let j = 1; j <= this.logFactor; j ++) {
      for (let i = 0; i + (1 << j) - 1 < elemCount; i ++) {
        this.cont[i][j] = merger(this.cont[i][j - 1], this.cont[i + (1 << (j - 1))][j - 1]);
      }
    }
  }
  query(leftBound: number, rightBound: number) {
